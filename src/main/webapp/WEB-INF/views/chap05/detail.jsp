...
<body>
    ...
    <script>
        ...
        function fetchGetReplies(pageNum = 1) { // 댓글 목록 요청시 페이지번호 전달(전달되지 않으면 기본값 1)
            // fetch 함수를 통해 비동기 통신 진행할 때 GET 요청은 요청에 관련한 객체를 따로 전달하지 않습니다.
            // method를 get이라고 얘기하지 않고, 데이터 전달 시에는 URL에 포함시켜서 전달.
            // 자바스크립트 문자열 안에 달러와 중괄호를 쓰면 el로 인식, 템플릿 리터럴 문자를 쓰고 싶으면 앞에 \를 붙여 주세요.
            // fetch(URL + '/' + bno)라고 해도됨.
            fetch(`\${URL}/\${bno}/page/\${pageNum}`)
                .then(res => res.json())
                .then(replyList => {
                    console.log(replyList);
                    // 서버로부터 전달받은 댓글 목록들을 화면에 그려야 한다.
                    // 기존에는 model에 담아서 jsp로 전달했고, jsp쪽에서 el을 이용해서 화면을 뿌렸음.
                    // 이제 서버는 그냥 데이터만 던져주고 끝임.
            ...
            $replyData.onclick = e => {
                ...
                if (e.target.matches('#replyDelBtn')) {
                    ...
                    */
                    ...
                    // 읽어온 댓글 내용을 모달 바디에 집어 넣기
                    document.getElementById('modReplyText').value = replyText;

                    // 아까 읽어놓은 댓글 번호도 모달 안에 있는 input hidden에 집어넣자
                    document.getElementById('modReplyId').value = rno;

                }
                // 저 두개 이외의 이벤트가 발생하면 걍 무시. -> return 굳이 쓸 필요 없음.
